...
4. **脚本会自动执行以下操作**：
   ...
   - 🔍 **规则一致性预检查** - 自动运行 `scripts/validate-rules-consistency.ps1` 验证规则元数据一致性
...
MCP跨平台同步工具 (`mcp-cross-platform-sync.ps1`) 是一个强大的配置管理工具，可以在不同的AI开发平台之间同步MCP服务器配置。
...
- 📊 **状态监控** - 实时查看各平台的配置状态
- 🛡️ **安全备份** - 同步前自动备份原有配置
- 🎛️ **灵活控制** - 支持单向、多向同步和预览模式

### **使用方法**

#### **1. 检测已安装的平台**
```powershell
# 检测所有已安装的AI平台及其MCP配置状态
...
```
🔍 检测到的AI平台：
✅ Trae AI - 已配置 (14个MCP服务器)
✅ Cursor IDE - 已配置 (14个MCP服务器)  
✅ OpenAI Codex - 已配置 (14个MCP服务器)
...
# 同步到多个平台
.\mcp-cross-platform-sync.ps1 sync -SourcePlatform "Trae" -TargetPlatforms "Cursor,OpenAICodex"
```

#### **3. 自动模式同步**
```powershell
# 自动选择配置最完整的平台作为源，同步到指定目标
.\mcp-cross-platform-sync.ps1 sync -TargetPlatforms "OpenAICodex"
```
...
# 2. 从已配置的平台同步到新平台
.\mcp-cross-platform-sync.ps1 sync -SourcePlatform "Trae" -TargetPlatforms "Cursor"
```

#### **场景2：配置标准化**
确保所有平台使用相同的MCP配置：
```powershell
# 使用最完整的配置作为标准，同步到所有其他平台
.\mcp-cross-platform-sync.ps1 sync -TargetPlatforms "Cursor,OpenAICodex"
```

#### **场景3：配置备份和恢复**
```powershell
# 同步前会自动创建备份，位于各平台配置目录的backup文件夹中
# 如需恢复，可以手动从备份文件夹复制配置文件
```

### **安全特性**

- 🛡️ **自动备份** - 每次同步前自动备份目标平台的原有配置
- 🔍 **预览模式** - 支持DryRun模式，可以预览同步操作而不实际执行
- ✅ **配置验证** - 同步后自动验证配置文件的有效性
- 📝 **详细日志** - 提供详细的操作日志和错误信息
...
⚠️ **使用前须知：**
- 确保目标平台已正确安装并至少运行过一次
- 同步会覆盖目标平台的现有MCP配置（但会先备份）
- 建议先使用 `-DryRun` 参数预览同步操作
- 同步后需要重启相应的AI平台以使配置生效

💡 **最佳实践：**
- 定期使用 `detect` 命令检查配置一致性
- 在重要操作前使用预览模式
- 保持一个平台作为"主配置"，定期同步到其他平台
