...
2. **导航到脚本所在文件夹**
   ...
   - 例如：`E:\AgentRules\English\rules-2.1-optimized-2.2.1\mcp-scripts`
   - 按回车键进入该文件夹
...
powershell -ExecutionPolicy Bypass -File "E:\AgentRules\English\rules-2.1-optimized-2.2.1\mcp-scripts\install-mcp-tools-enhanced-final.ps1"
```
...
- `install-mcp-tools-simple.bat` - 简化批处理脚本（备用）
- `install-mcp-tools-enhanced-fixed.bat` - 修复版批处理脚本（备用）

### **配置和文档**
- `安装脚本使用说明.md` - 本文件
- `mcp-config.json` - 安装后生成的配置文件
