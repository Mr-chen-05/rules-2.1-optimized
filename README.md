...
|------|------|------|
| 🚀 **快速开始** | 👇 下方快速开始部分 | 3分钟上手 |
| 📖 **详细使用** | [USAGE.md](USAGE.md) | 高级配置和命令参考 |
| ⚡ **命令参考** | [COMMANDS-REFERENCE.md](COMMANDS-REFERENCE.md) | 所有可用命令 (建议完成入门配置后查看) |
| 🔧 **MCP配置** | [MCP快速入门](docs/MCP-QUICK-START-GUIDE.md) | 5分钟配置MCP工具 |
...
- 🔍 **找不到MCP工具？** → 运行 `docs/find-mcp-tools.bat` 自动查找
...
│   │   └── find-mcp-tools.bat              # 🔍 MCP工具自动查找脚本
│   └── USAGE.md                            # 详细使用指南
│
├── 🚀 安装脚本（企业级测试通过）
│   ├── install-scripts/
...
### 🔧 MCP工具支持
| 工具名称 | 功能详述 | 主要用途 | 状态 | 安装方式 |
|----------|----------|----------|------|----------|
| **mcp-feedback-enhanced** | 智能反馈增强 | 用户交互、进度汇报、决策确认 | ✅ 可用 | `uvx mcp-feedback-enhanced@latest` |
| **server-filesystem** | 文件系统操作 | 文件读写、目录管理、批量操作 | ✅ 可用 | `npx @modelcontextprotocol/server-filesystem` |
| **server-memory** | 记忆管理 | 信息存储、知识图谱、上下文管理 | ✅ 可用 | `npx @modelcontextprotocol/server-memory` |
| **server-github** | GitHub集成 | 仓库管理、代码提交、PR处理 | ✅ 可用 | `npx @modelcontextprotocol/server-github` |
| **server-everything** | 通用工具集 | 基础操作、测试验证、开发辅助 | ✅ 可用 | `npx @modelcontextprotocol/server-everything` |

> **📚 详细功能说明**: 查看 [MCP工具详细功能参考](docs/MCP-TOOLS-REFERENCE.md) 了解每个工具的具体功能和最佳实践
>
> **💡 新手提示**: 建议先完成上方的快速开始和相关教程，熟悉基本使用后再查看详细功能参考
