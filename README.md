...
| `/component` | 组件创建 | 创建UI组件 | 无独立 .mdc 文件（自然语言触发） |
| `/ui-design` | UI设计 | 界面设计和样式 | 无独立 .mdc 文件（自然语言触发） |
| `/api-design` | API设计 | 接口设计和文档 | 无独立 .mdc 文件（自然语言触发） |
| `/database-design` | 数据库设计 | 数据模型设计 | 无独立 .mdc 文件（自然语言触发） |
| `/responsive-design` | 响应式设计 | 移动端适配 | 无独立 .mdc 文件（自然语言触发） |
